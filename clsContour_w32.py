...
class MakeContourByMesh():
    ...
    def __init__(
        ...
        # グラフ
        self.PyGraph_DrawImage = self.g_shmlib.PyGraph_DrawImage
        self.PyGraph_DrawImage.argtypes = [c_char_p, c_int, c_int]
        self.PyGraph_DrawImage.restype = c_int

        self.PyGraph_DrawLine = self.g_shmlib.PyGraph_DrawLine
        self.PyGraph_DrawLine.argtypes = [c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_DrawLine.restype = c_int

        self.PyGraph_DrawEllipse = self.g_shmlib.PyGraph_DrawEllipse
        self.PyGraph_DrawEllipse.argtypes = [c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_DrawEllipse.restype = c_int

        self.PyGraph_DrawPolygon = self.g_shmlib.PyGraph_DrawPolygon
        self.PyGraph_DrawPolygon.argtypes = [c_char_p, c_int, c_int, c_int, c_int]
        self.PyGraph_DrawPolygon.restype = c_int

        self.PyGraph_DrawRectangle = self.g_shmlib.PyGraph_DrawRectangle
        self.PyGraph_DrawRectangle.argtypes = [c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_DrawRectangle.restype = c_int

        self.PyGraph_DrawString = self.g_shmlib.PyGraph_DrawString
        self.PyGraph_DrawString.argtypes = [c_char_p, c_char_p, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_DrawString.restype = c_int

        self.PyGraph_FillEllipse = self.g_shmlib.PyGraph_FillEllipse
        self.PyGraph_FillEllipse.argtypes = [c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_FillEllipse.restype = c_int

        self.PyGraph_FillPolygon = self.g_shmlib.PyGraph_FillPolygon
        self.PyGraph_FillPolygon.argtypes = [c_char_p, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_FillPolygon.restype = c_int

        self.PyGraph_FillRectangle = self.g_shmlib.PyGraph_FillRectangle
        self.PyGraph_FillRectangle.argtypes = [c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int, c_int]
        self.PyGraph_FillRectangle.restype = c_int

        self.PyGraph_Init = self.g_shmlib.PyGraph_Init
        self.PyGraph_Init.argtypes = [c_int, c_int]
        self.PyGraph_Init.restype = c_int

        self.PyGraph_Save = self.g_shmlib.PyGraph_Save
        self.PyGraph_Save.argtypes = [c_char_p]
        self.PyGraph_Save.restype = c_int

        self.PyGraph_Term = self.g_shmlib.PyGraph_Term
        self.PyGraph_Term.argtypes = []
        self.PyGraph_Term.restype = c_int
    ...
    def _drawContour(
        ...
        try:
            ...
            # X軸、Y軸のセパレートを設定する。
            a_xSep = (h_img_width - (self.prv_leftMargin + self.prv_rightMargin)) / a_xMax
            a_ySep = (h_img_height - (self.prv_topMargin + self.prv_bottomMargin)) / a_yMax
            ...
            self.PyGraph_DrawString(
                c_char_p(a_msno5km.encode("shift-jis")),
                c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                int(h_img_width / 2 - 40), 2,
                0, 0, 0
            )
            self.PyGraph_DrawString(
                c_char_p(a_msno1km.encode("shift-jis")),
                c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                int(h_img_width / 2 - 40), 12,
                0, 0, 0
            )
            ...
            for a_cnt1 in range(0, a_yLine + 1):
                ...
                if (a_cnt1 >= 1) and (a_cnt1 < a_yLine):
                    a_pen_style = 2 #Drawing2D.DashStyle.Dot
                    a_pen_color = (192, 192, 192)
                else:
                    a_pen_style = 0 #Drawing2D.DashStyle.Solid
                    a_pen_color = (0, 0, 0)

                if (a_bFlag == True):
                    self.PyGraph_DrawLine(
                        int(self.prv_leftMargin), int(self.prv_topMargin + (a_ySep * (self.com.g_yUnit * a_cnt1))),
                        int(h_img_width - self.prv_rightMargin), int(self.prv_topMargin + (a_ySep * (self.com.g_yUnit * a_cnt1))),
                        a_pen_style, a_pen_width,
                        a_pen_color[0], a_pen_color[1], a_pen_color[2]
                    )
                ...
                self.PyGraph_DrawString(
                    c_char_p(a_strTmp.encode("shift-jis")),
                    c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                    a_CX, int(h_img_height - self.prv_bottomMargin - (a_ySep * (self.com.g_yUnit * a_cnt1)) - 5.333333333333333),
                    0, 0, 0
                )
            ...
            for a_cnt1 in range(0, a_xLine + 1):
                ...
                self.PyGraph_DrawString(
                    c_char_p(a_strTmp.encode("shift-jis")),
                    c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                    a_CX, int(h_img_height - self.prv_bottomMargin  + 2.6666666666666665),
                    0, 0, 0
                )

            # 文字の設定
            self.PyGraph_DrawImage(
                c_char_p(".\\images\\imgTitleSoil.bmp".encode("shift-jis")),
                int(h_img_width / 2 - 46.666666666666664), int(h_img_height - (self.prv_bottomMargin / 2) - 5.333333333333333)
            )
            self.PyGraph_DrawImage(
                c_char_p(".\\images\\imgTitleRain.bmp".encode("shift-jis")),
                int(6.666666666666667), int((h_img_height - self.prv_bottomMargin) / 2 - 100.0)
            )
            ...
            for a_cnt1 in range(1, 10):
                            ...
                            if (a_IsSet == False):
                                if (float(a_split1[0]) == soilMin):
                                    if (a_cnt1 == a_TargetCL):
                                        ...
                                        a_TanbuY1 = int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))) + 10)
                                    if (a_cnt1 == 9):
                                        a_TanbuX2 = int(self.prv_leftMargin + float(a_split1[0]) * a_xSep)
                                        a_TanbuY2 = int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))))

                                    # 下限値と同じ場合は上部へ伸びる線を描画
                                    if (rainMax != -1):
                                        if (a_TargetCL < 0) or (a_cnt1 == a_TargetCL):
                                            self.PyGraph_DrawLine(
                                                int(self.prv_leftMargin + float(0) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * rainMax))),
                                                int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * rainMax))),
                                                0, a_pen_width,
                                                a_pen_color[0], a_pen_color[1], a_pen_color[2]
                                            )
                                            self.PyGraph_DrawLine(
                                                int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1])))),
                                                int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * rainMax))),
                                                0, a_pen_width,
                                                a_pen_color[0], a_pen_color[1], a_pen_color[2]
                                            )
                                    ...
                                    if (a_cnt2 > 0):
                                        ...
                                        if (rainMax != -1):
                                            ...
                                            self.PyGraph_DrawLine(
                                                int(self.prv_leftMargin + float(soilMin) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_splitTmp[10 - a_cnt1])))),
                    ...
                    if (a_setSoilMin == False):
                        if (float(a_split1[10 - a_cnt1]) > 0):
                            if (float(a_split2[10 - a_cnt1]) > 0):
                                self.PyGraph_DrawLine(
                                    int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1])))),
                                    int(self.prv_leftMargin + float(a_split2[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split2[10 - a_cnt1])))),
                                    0, a_pen_width,
                                    a_pen_color[0], a_pen_color[1], a_pen_color[2]
                                )
                            else:
                                ...
                                self.PyGraph_DrawLine(
                                    int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1])))),
                                    int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split2[10 - a_cnt1])))),
                                    0, a_pen_width,
                                    a_pen_color[0], a_pen_color[1], a_pen_color[2]
                                )
                        else:
                ...
                if (a_split2 != None):
                    if (float(a_split2[10 - a_cnt1]) > 0):
                        self.PyGraph_DrawLine(
                            int(self.prv_leftMargin + float(a_split2[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split2[10 - a_cnt1])))),
                            int(self.prv_leftMargin + float(a_split2[0]) * a_xSep), int(h_img_height - (self.prv_bottomMargin + (a_ySep * float(0)))),
                            0, a_pen_width,
                            a_pen_color[0], a_pen_color[1], a_pen_color[2]
                        )
            ...
            if (a_TanbuX1 > 0):
                ...
                self.PyGraph_DrawLine(
                    ...
                    0, a_pen_Tanbu_width,
                    a_pen_Tanbu_color[0], a_pen_Tanbu_color[1], a_pen_Tanbu_color[2]
                )
                ...
                if (a_isDrawTargetCLOnly == False):
                    ...
                    self.PyGraph_DrawLine(
                        int(a_TanbuX1), int(self.prv_topMargin),
                        int(a_TanbuX2), int(a_TanbuY2),
                        0, a_pen_Tanbu_width,
                        a_pen_Tanbu_color[0], a_pen_Tanbu_color[1], a_pen_Tanbu_color[2]
                    )
            ...
            if ((outAction == self.com.g_Action_MakeContourSnake) or (outAction == self.com.g_Action_MakeContourSoilMin)):
                ...
                if (a_isDrawNonOccurRainfall == True):
                    self._drawNonOccurRainFall(h_img_width, h_img_height, a_xSep, a_ySep)

                # 発生降雨の描画
                self._drawOccurRainFall(h_img_width, h_img_height, a_xSep, a_ySep, a_occurColor, a_occurTime, a_occurMark)
                # Unrealの描画
                self._drawUnreal(h_img_width, h_img_height, a_xLine, a_yLine, a_xSep, a_ySep, unReal)

                # 災害発生のデータを考慮した描画とする。→2006.03.20
                self._drawLegendS(h_img_width, h_img_height, a_occurColor, a_occurTime, a_occurMark, a_drawInfo)
            else:
                self.PyGraph_DrawImage(
                    c_char_p(".\\images\\imgLegendC.bmp".encode("shift-jis")),
                    int(h_img_width - self.prv_rightMargin + 5.333333333333333), int(self.prv_topMargin)
                )
    ...
    def _drawLegendS(
        ...
        try:
            ...
            self.PyGraph_FillRectangle(
                int(h_img_width - self.prv_rightMargin - 33.3333333333333), int(self.prv_topMargin + 6.66666666666667),
                int(h_img_width - 6.66666666666667), int((self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + (a_textSum + 1))))),
                255, 255, 255,
                0, 0, 0
            )
            '''
            self.PyGraph_DrawRectangle(
                int(h_img_width - self.prv_rightMargin - 33.3333333333333), int(self.prv_topMargin + 6.66666666666667),
                int(h_img_width - 6.66666666666667), int((self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + (a_textSum + 1))))),
                a_pen_width,
                0, 0, 0
            ...
            for a_cnt1 in range(1, 10):
                ...
                self.PyGraph_DrawLine(
                    int((h_img_width - self.prv_rightMargin) - 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * a_cnt1)),
                    int((h_img_width - self.prv_rightMargin) + 13.3333333333333), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * a_cnt1)),
                    0, a_pen_width2,
                    a_pen_color2[0], a_pen_color2[1], a_pen_color2[2]
                )
                self.PyGraph_DrawString(
                    c_char_p(str(a_cnt1 / 10).encode("shift-jis")),
                    c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                    int((h_img_width - self.prv_rightMargin) + 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * a_cnt1) - 4),
                    0, 0, 0
                )

            # 非発生降雨の凡例
            # EllipseのX,Yは左上角の位置である。
            self.PyGraph_FillEllipse(
                int(h_img_width - self.prv_rightMargin - 1), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * 10) - 1),
                int(h_img_width - self.prv_rightMargin + 1), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * 10) + 1),
                0, 64, 128,
                0, 64, 128
            )
            self.PyGraph_DrawString(
                c_char_p("非発生降雨".encode("shift-jis")),
                c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                int((h_img_width - self.prv_rightMargin) + 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * 10) - 4),
                0, 0, 0
            )
            ...
            for a_cnt in range(0, len(h_occurTime)):
                if (len(h_occurTime[a_cnt]) > 0):
                    ...
                    for a_strTmp in a_textline:
                        a_x1 = int((h_img_width - self.prv_rightMargin) - 20)
                        a_y1 = int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)))
                        a_x2 = int((h_img_width - self.prv_rightMargin) + 13.3333333333333)
                        a_y2 = int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)))
                        a_x3 = a_x1 + ((a_x2 - a_x1) / 2)

                        self.PyGraph_DrawLine(
                            int((h_img_width - self.prv_rightMargin) - 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1))),
                            int((h_img_width - self.prv_rightMargin) + 13.3333333333333), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1))),
                            0, a_pen_width2,
                            a_pen_color2[0], a_pen_color2[1], a_pen_color2[2]
                        )
                        self.PyGraph_DrawString(
                            c_char_p(str(a_strTmp + " 災害").encode("shift-jis")),
                            c_char_p("MS UI Gothic".encode("shift-jis")), 9,
                            int((h_img_width - self.prv_rightMargin) + 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)) - 4),
                            0, 0, 0
                        )
                        ...
                        for a_iCnt3 in range(0, len(h_occurMark)):
                            ...
                            if (a_textline3[0] == a_strTmp):
                                ...
                                if (a_iTmp == 1):   # 赤○（外側は黒線）
                                    # EllipseのX,Yは左上角の位置である。
                                    self.PyGraph_FillEllipse(
                                        int(float(a_x3) - 6.666666666666667), int(float(a_y1) - 6.666666666666667),
                                        int(float(a_x3) + 6.666666666666667), int(float(a_y1) + 6.666666666666667),
                                        255, 0, 0,
                                        0, 0, 0
                                    )
                                elif (a_iTmp == 2):   # 青◇（外側は黒線）
                                    a_points = str(int(float(a_x3))) + "," + str(int(float(a_y1) + 6.666666666666667))
                                    a_points += "," + str(int(float(a_x3) - 6.666666666666667)) + "," + str(int(float(a_y1)))
                                    a_points += "," + str(int(float(a_x3))) + "," + str(int(float(a_y1) - 6.666666666666667))
                                    a_points += "," + str(int(float(a_x3) + 6.666666666666667)) + "," + str(int(float(a_y1)))
                                    #a_points += "," + str(int(float(a_x3))) + "," + str(int(float(a_y1) + 6.666666666666667))
                                    self.PyGraph_FillPolygon(
                                        c_char_p(a_points.encode("shift-jis")),
                                        0, 0, 255,
                                        0, 0, 0
                                    )
                                elif (a_iTmp == 3):   # 緑△（外側は黒線）
                                    a_points = str(int(float(a_x3) - 6.666666666666667)) + "," + str(int(float(a_y1) + 6.666666666666667))
                                    a_points += "," + str(int(float(a_x3))) + "," + str(int(float(a_y1) - 6.666666666666667))
                                    a_points += "," + str(int(float(a_x3) + 6.666666666666667)) + "," + str(int(float(a_y1) + 6.666666666666667))
                                    #a_points += "," + str(int(float(a_x3) - 6.666666666666667)) + "," + str(int(float(a_y1) + 6.666666666666667))
                                    self.PyGraph_FillPolygon(
                                    ...
                                    self.PyGraph_DrawString(
                                        c_char_p("×".encode("shift-jis")),
                                        c_char_p("MS UI Gothic".encode("shift-jis")), 18,
                                        int(float(a_x3)), int(float(a_y1)),
                                        255, 0, 0
                                    )
                                elif (a_iTmp == 6):   # 青＊
                                    self.PyGraph_DrawString(
        ...
        try:
            ...
            for a_cnt in range(int(self.StartYear), int(self.EndYear) + 1):
                ...
                for a_cnt2 in range(1, a_textSum):
                    ...
                    # EllipseのX,Yは左上角の位置である。
                    self.PyGraph_FillEllipse(
                        int(self.prv_leftMargin + (float(a_split[7]) * xSep) - 1), int(h_img_height - (self.prv_bottomMargin + (float(a_split[6]) * ySep)) - 1),
                        int(self.prv_leftMargin + (float(a_split[7]) * xSep) + 1), int(h_img_height - (self.prv_bottomMargin + (float(a_split[6]) * ySep)) + 1),
                        0, 64, 128,
                        0, 64, 128
                    )
                ...
                for a_cnt1 in range(1, a_textSum):
                    ...
                    if (float(a_split[6]) > 0):
                        if (a_prevX != -999):
                            # 発生降雨を線で描画する
                            self.PyGraph_DrawLine(
                                int(self.prv_leftMargin + a_prevX * xSep), int(h_img_height - (self.prv_bottomMargin + (ySep * a_prevY))),
                                int(self.prv_leftMargin + float(a_split[7]) * xSep), int(h_img_height - (self.prv_bottomMargin + (ySep * float(a_split[6])))),
                                0, a_pen_width,
                                a_pen_color[0], a_pen_color[1], a_pen_color[2]
                            )
                            a_IsPlot = True
                    ...
                    for a_cnt2 in range(0, a_textSum2):
                        ...
                        if (a_split[2] == a_split2[1].strip()) and (a_split[3] == a_split2[2].strip()) and (a_split[4] == a_split2[3].strip()) and (a_split[5] == a_split2[4].strip()):
                            ...
                            a_textline2[a_cnt2] = \
                                ...
                                "," + str(self.prv_leftMargin + float(a_split[7]) * xSep) + "," + str(h_img_height - (self.prv_bottomMargin + (ySep * float(a_split[6]))))
                ...
                if (len(a_split2) >= 8):
                    if (a_split2[6] != "") and (a_split2[7] != ""):
                        ...
                        if (a_mark == 1):   # 赤○（外側は黒線）
                            ...
                            a_points = str(int(float(a_split2[6]))) + "," + str(int(float(a_split2[7]) + 6.666666666666667))
                            a_points += "," + str(int(float(a_split2[6]) - 6.666666666666667)) + "," + str(int(float(a_split2[7])))
                            a_points += "," + str(int(float(a_split2[6]))) + "," + str(int(float(a_split2[7]) - 6.666666666666667))
                            a_points += "," + str(int(float(a_split2[6]) + 6.666666666666667)) + "," + str(int(float(a_split2[7])))
                            #a_points += "," + str(int(float(a_split2[6]))) + "," + str(int(float(a_split2[7]) + 6.666666666666667))
                            self.PyGraph_FillPolygon(
                            ...
                            a_points = str(int(float(a_split2[6]) - 6.666666666666667)) + "," + str(int(float(a_split2[7]) + 6.666666666666667))
                            a_points += "," + str(int(float(a_split2[6]))) + "," + str(int(float(a_split2[7]) - 6.666666666666667))
                            a_points += "," + str(int(float(a_split2[6]) + 6.666666666666667)) + "," + str(int(float(a_split2[7]) + 6.666666666666667))
                            #a_points += "," + str(int(float(a_split2[6]) - 6.666666666666667)) + "," + str(int(float(a_split2[7]) + 6.666666666666667))
                            self.PyGraph_FillPolygon(
                            ...
                            self.PyGraph_DrawString(
                                c_char_p("×".encode("shift-jis")),
                                c_char_p("MS UI Gothic".encode("shift-jis")), 18,
                                int(float(a_split2[6])), int(float(a_split2[7])),
                                255, 0, 0
                            )
                        elif (a_mark == 6):   # 青＊
                            self.PyGraph_DrawString(
    ...
    def _drawUnreal(
        ...
        try:
            ...
            a_points = str(int(self.prv_leftMargin)) + "," + str(int(h_img_height - self.prv_bottomMargin))
            a_points += "," + str(int(self.prv_leftMargin)) + "," + str(int(h_img_height - (self.prv_bottomMargin + (ySep * (self.com.g_yUnit * yLine)))))
            a_points += "," + str(int(self.prv_leftMargin + xSep * ((self.com.g_yUnit * yLine) / a_unReal))) + "," + str(int(h_img_height - (self.prv_bottomMargin + (ySep * (self.com.g_yUnit * yLine)))))
            #a_points += "," + str(int(self.prv_leftMargin)) + "," + str(int(h_img_height - self.prv_bottomMargin))
            ...
            self.PyGraph_DrawImage(
                c_char_p(".\\images\\imgUnreal.bmp".encode("shift-jis")),
    ...
    def _makeAutoContourOrigin(
        ...
        try:
            self.PyGraph_Init(
                int(self.com.g_ImageFileWidth),
                int(self.com.g_ImageFileHeight)
            )

            self._makeContourOriginEx()
            self._drawContour(self.com.g_ContourOriginSymbol, self.com.g_Action_MakeContourOrigin, int(self.com.g_ImageFileWidth), int(self.com.g_ImageFileHeight), -1, 0, -1, 0, h_meshNo)
            self._saveContourImage(self.com.g_ContourOriginSymbol)
    ...
    def _makeAutoContourRevise(
        ...
        try:
            ...
            self._contourRevise()
            self._drawContour(self.com.g_ContourReviseSymbol, self.com.g_Action_MakeContourRevise, int(self.com.g_ImageFileWidth), int(self.com.g_ImageFileHeight), -1, 0, -1, 0, h_meshNo)
            self._saveContourImage(self.com.g_ContourReviseSymbol)
    ...
    def _makeAutoContourSnake(
        ...
        try:
            ...
            if (self.com.g_PastKind == 0):
                # 取り込みなし
                #a_i = 0
                self._drawContour(self.com.g_ContourReviseSymbol, self.com.g_Action_MakeContourSnake, int(self.com.g_ImageFileWidth), int(self.com.g_ImageFileHeight), -1, 0, -1, 0, h_meshNo)
                self._saveContourImage(self.com.g_ContourSnakeSymbol)
            else:
                # 取り込みあり
                a_RBFN = 0
                a_soilMin = 0
                ...
                self._drawContour(self.com.g_ContourReviseSoilMinSymbol, self.com.g_Action_MakeContourSoilMin, int(self.com.g_ImageFileWidth), int(self.com.g_ImageFileHeight), a_dRBFN, a_soilMin, a_rainMax, 0, h_meshNo)   # 60分間積算雨量上限値のサポート
                self._saveContourImage(self.com.g_ContourSnakeSymbol)
    ...
    # 等高線の保存
    def _saveContourImage(self, h_symbol):
        a_strErr = "h_symbol=" + h_symbol
        self.com.Outputlog(self.com.g_LOGMODE_TRACE1, '_saveContourImage', a_strErr)

        try:
            a_fname = self.com.g_OutPath + "\\" + h_symbol + "-" + self.TargetMeshNo + "-" + str(self.StartYear) + "-" + str(self.EndYear) + ".bmp"
            self.PyGraph_Save(
                a_fname.encode("shift-jis")
            )
            self.PyGraph_Term()
