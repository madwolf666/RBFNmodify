...
class MakeContourByMesh():
    ...
    def _drawContour(
        ...
        try:
            a_img_height, a_img_width = h_img.shape[:2]    # サイズを取得
            ...
            # X軸、Y軸のセパレートを設定する。
            a_xSep = (a_img_width - (self.prv_leftMargin + self.prv_rightMargin)) / a_xMax
            a_ySep = (a_img_height - (self.prv_topMargin + self.prv_bottomMargin)) / a_yMax
            ...
            for a_cnt1 in range(0, a_yLine + 1):
                ...
                if (a_bFlag == True):
                    cv2.line(
                        ...
                        (int(a_img_width - self.prv_rightMargin), int(self.prv_topMargin + (a_ySep * (self.com.g_yUnit * a_cnt1)))),
                        a_pen_color,
                        a_pen_width
                    )
                ...
                cv2.putText(
                    h_img,
                    a_strTmp,
                    (a_CX, int(a_img_height - self.prv_bottomMargin - (a_ySep * (self.com.g_yUnit * a_cnt1)) - 5.333333333333333)),
            ...
            for a_cnt1 in range(0, a_xLine + 1):
                ...
                cv2.putText(
                    h_img,
                    a_strTmp,
                    (a_CX, int(a_img_height - self.prv_bottomMargin  + 2.6666666666666665)),
                    cv2.FONT_HERSHEY_PLAIN,
                    a_fnt_font_size,
                    a_fnt_color
                )
            ...
            # 文字の設定
            # 画像を重ねる⇒OpenCVはGIFに対応していない
            #a_imgSoil = cv2.imread(".\\images\\imgTitleSoil.gif")
            #a_imgRain = cv2.imread(".\\images\\imgTitleRain.gif")
            a_imgSoil = cv2.imread(".\\images\\imgTitleSoil.png")
            a_imgRain = cv2.imread(".\\images\\imgTitleRain.png")
            a_height, a_width = a_imgSoil.shape[:2]    # サイズを取得
            h_img[
            int(a_img_height - (self.prv_bottomMargin / 2) - 5.333333333333333):int(a_img_height - (self.prv_bottomMargin / 2) - 5.333333333333333) + a_height,
            int(a_img_width / 2 - 46.666666666666664):int(a_img_width / 2 - 46.666666666666664) + a_width
            ] = a_imgSoil
            a_height, a_width = a_imgRain.shape[:2]    # サイズを取得
            h_img[
            int((a_img_height - self.prv_bottomMargin) / 2 - 100.0):int((a_img_height - self.prv_bottomMargin) / 2 - 100.0) + a_height,
            int(6.666666666666667):int(6.666666666666667) + a_width
            ] = a_imgRain
            ...
            for a_cnt1 in range(1, 10):
                ...
                if (len(a_drawInfo) > 0):
                    ...
                    a_pen_color = (int(a_splitC[2]), int(a_splitC[1]), int(a_splitC[0]))    # BGR
                    #a_pen_color = (int(a_splitC[0]), int(a_splitC[1]), int(a_splitC[2]))   # RGB
                    ...
                    if (a_cnt1 == 1):   # 0.1
                        ...
                        a_pen_color = (192, 128, 0)
                        #a_pen_color = (0, 128, 192)
                    elif (a_cnt1 == 7):   # 0.7
                        a_pen_color = (0, 255, 255)
                            ...
                            if (a_IsSet == False):
                                if (float(a_split1[0]) == soilMin):
                                    if (a_cnt1 == a_TargetCL):
                                        ...
                                        a_TanbuY1 = int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))) + 10)
                                    if (a_cnt1 == 9):
                                        a_TanbuX2 = int(self.prv_leftMargin + float(a_split1[0]) * a_xSep)
                                        a_TanbuY2 = int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))))
                                    ...
                                    if (rainMax != -1):
                                        if (a_TargetCL < 0) or (a_cnt1 == a_TargetCL):
                                            cv2.line(
                                                h_img,
                                                (int(self.prv_leftMargin + float(0) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * rainMax)))),
                                                (int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * rainMax)))),
                                                a_pen_color,
                                                a_pen_width
                                            )
                                            cv2.line(
                                                h_img,
                                                (int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))))),
                                                (int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * rainMax)))),
                                                a_pen_color,
                                                a_pen_width
                                            )
                                    ...
                                    if (a_cnt2 > 0):
                                        ...
                                        if (rainMax != -1):
                                            ...
                                            cv2.line(
                                                h_img,
                                                (int(self.prv_leftMargin + float(soilMin) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_splitTmp[10 - a_cnt1]))))),
                        ...
                        if (float(a_split1[10 - a_cnt1]) > 0):
                            if (float(a_split2[10 - a_cnt1]) > 0):
                                cv2.line(
                                    h_img,
                                    (int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))))),
                                    (int(self.prv_leftMargin + float(a_split2[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split2[10 - a_cnt1]))))),
                                    a_pen_color,
                                    a_pen_width
                                )
                            else:
                                ...
                                cv2.line(
                                    h_img,
                                    (int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split1[10 - a_cnt1]))))),
                                    (int(self.prv_leftMargin + float(a_split1[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split2[10 - a_cnt1]))))),
                                    a_pen_color,
                                    a_pen_width
                                )
                        else:
                ...
                if (a_split2 != None):
                    if (float(a_split2[10 - a_cnt1]) > 0):
                        cv2.line(
                            h_img,
                            (int(self.prv_leftMargin + float(a_split2[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(a_split2[10 - a_cnt1]))))),
                            (int(self.prv_leftMargin + float(a_split2[0]) * a_xSep), int(a_img_height - (self.prv_bottomMargin + (a_ySep * float(0))))),
                            a_pen_color,
                            a_pen_width
                        )
            ...
            if (a_TanbuX1 > 0):
                ...
                if (a_isDrawTargetCLOnly == False):
                    if (len(a_drawInfo) > 0):
                        ...
                        a_pen_Tanbu_color = (int(a_splitC[2]), int(a_splitC[1]), int(a_splitC[0]))
            ...
            if ((outAction == self.com.g_Action_MakeContourSnake) or (outAction == self.com.g_Action_MakeContourSoilMin)):
                ...
                if (a_isDrawNonOccurRainfall == True):
                    self._drawNonOccurRainFall(h_img, a_xSep, a_ySep)

                # 発生降雨の描画
                self._drawOccurRainFall(h_img, a_xSep, a_ySep, a_occurColor, a_occurTime, a_occurMark)
                # Unrealの描画
                self._drawUnreal(h_img, a_xLine, a_yLine, a_xSep, a_ySep, unReal)

                # 災害発生のデータを考慮した描画とする。→2006.03.20
                self._drawLegendS(h_img, a_occurColor, a_occurTime, a_occurMark, a_drawInfo)
            else:
                # 画像を重ねる
                #a_imgL = cv2.imread(".\\images\\imgLegendC.gif")
                a_imgL = cv2.imread(".\\images\\imgLegendC.png")
                a_height, a_width = a_imgL.shape[:2]    # サイズを取得
                h_img[
                int(self.prv_topMargin):int(self.prv_topMargin) + a_height,
                int(a_img_width - self.prv_rightMargin + 5.333333333333333):int(a_img_width - self.prv_rightMargin + 5.333333333333333) + a_width
                ] = a_imgL
    ...
    def _drawLegendS(
        ...
        try:
            ...
            a_fill = (255, 255, 255)
            a_fill2 = (0, 0, 0)
            a_fill3 = (128, 64, 0)  # BGR
            #a_fill3 = (0, 64, 128) # RGB
            a_pen_color = (0, 0, 0)
            a_pen_width = 1
            a_pen_color2 = (0, 0, 0)
            a_pen_width2 = 1
            ...
            a_pt = np.array(
                [
                    [int(a_img_width - self.prv_rightMargin - 33.3333333333333), int(self.prv_topMargin + 6.66666666666667)],
                    [int(a_img_width - 6.66666666666667), int(self.prv_topMargin + 6.66666666666667)],
                    [int(a_img_width - 6.66666666666667), int((self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + (a_textSum + 1)))))],
                    [int(a_img_width - self.prv_rightMargin - 33.3333333333333), int((self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + (a_textSum + 1)))))]
                ]
            )
            cv2.fillConvexPoly(
                h_img,
            ...
            for a_cnt1 in range(1, 10):
                ...
                if (len(h_drawInfo) > 0):
                    ...
                    if (a_cnt1 == 1): # 0.1
                        ...
                        a_pen_color2 = (192, 128, 0)
                        #a_pen_color2 = (0, 128, 192)
                    elif (a_cnt1 == 7): # 0.7
                        a_pen_color2 = (0, 255, 255)
                ...
                cv2.line(
                    h_img,
                    (int((a_img_width - self.prv_rightMargin) - 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * a_cnt1))),
                    (int((a_img_width - self.prv_rightMargin) + 13.3333333333333), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * a_cnt1))),
                    a_pen_color2,
                    a_pen_width2
                )
                '''
                cv2.putText(
                    h_img,
                    str(a_cnt1 / 10),
                    (int((a_img_width - self.prv_rightMargin) + 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * a_cnt1) - 4)),
                    cv2.FONT_HERSHEY_PLAIN,
                    a_fnt_font_size,
                    (0, 0, 0)
                )
            ...
            cv2.circle(
                h_img,
                (int((a_img_width - self.prv_rightMargin) - 4), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * 10))),
                1,
                a_fill3,
                -1
            )
            ...
            cv2.putText(
                h_img,
                "非発生降雨",
                (int((a_img_width - self.prv_rightMargin) + 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * 10) - 4)),
                cv2.FONT_HERSHEY_PLAIN,
                a_fnt_font_size,
                (0, 0, 0)
            )
            ...
            for a_cnt in range(0, len(h_occurTime)):
                if (len(h_occurTime[a_cnt]) > 0):
                    a_pen_color2 = (occurColor[a_cnt][2], occurColor[a_cnt][1], occurColor[a_cnt][0])
                    ...
                    for a_strTmp in a_textline:
                        a_x1 = int((a_img_width - self.prv_rightMargin) - 20)
                        a_y1 = int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)))
                        a_x2 = int((a_img_width - self.prv_rightMargin) + 13.3333333333333)
                        a_y2 = int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)))
                        a_x3 = a_x1 + ((a_x2 - a_x1) / 2)
                        cv2.line(
                            h_img,
                            (int((a_img_width - self.prv_rightMargin) - 20), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)))),
                            (int((a_img_width - self.prv_rightMargin) + 13.3333333333333), int(self.prv_topMargin + 6.66666666666667 + (17.3333333333333 * (10 + a_iCnt2 + 1)))),
                            a_pen_color2,
                            a_pen_width2
                        )
                        '''
                        cv2.putText(
                            ...
                            a_strTmp + " 災害",
                            (
                                int((a_img_width - self.prv_rightMargin) + 20),
                            ...
                            if (a_textline3[0] == a_strTmp):
                                ...
                                if (a_iTmp == 1):   # 赤○（外側は黒線）
                                    ...
                                    cv2.putText(
                                        h_img,
                                        "×",
                                        (int(float(a_x3) - a_fnt_font2_size * 10), int(float(a_y1) - a_fnt_font2_size * 10)),
                                        cv2.FONT_HERSHEY_PLAIN,
                                        a_fnt_font2_size,
                                        (0, 0, 255)
                                    )
        ...
        try:
            ...
            for a_cnt in range(int(self.StartYear), int(self.EndYear) + 1):
                ...
                for a_cnt2 in range(1, a_textSum):
                    ...
                    # 非発生降雨を点で描画する
                    # EllipseのX,Yは左上角の位置である。
                    # 画像, 中央座標, 長短軸, 回転角度, 円弧開始角度, 円弧終了角度, 色, 線の太さ
                    cv2.circle(
                        h_img,
                        (int(self.prv_leftMargin + (float(a_split[7]) * xSep)), int(a_img_height - (self.prv_bottomMargin + (float(a_split[6]) * ySep)))),
                        1,
                        a_fill,
                        -1
                    )
    ...
    def _drawOccurRainFall(
        ...
        try:
            ...
            a_pen_color = (a_colorRGB[0][2], a_colorRGB[0][1], a_colorRGB[0][0])    # BGR
            #a_pen_color = (a_colorRGB[0][0], a_colorRGB[0][1], a_colorRGB[0][2])   # RGB
            ...
            for a_cnt in range(int(self.StartYear), int(self.EndYear) + 1):
                ...
                for a_cnt1 in range(1, a_textSum):
                    ...
                    if (a_nextLine != a_nowLine):
                        ...
                        a_pen_color = (a_colorRGB[a_nowLine - 1][2], a_colorRGB[a_nowLine - 1][1], a_colorRGB[a_nowLine - 1][0])    # BGR
                        #a_pen_color = (a_colorRGB[a_nowLine - 1][0], a_colorRGB[a_nowLine - 1][1], a_colorRGB[a_nowLine - 1][2])   # RGB
                    ...
                    if (float(a_split[6]) > 0):
                        if (a_prevX != -999):
                            ...
                            cv2.line(
                                h_img,
                                (int(self.prv_leftMargin + a_prevX * xSep), int(a_img_height - (self.prv_bottomMargin + (ySep * a_prevY)))),
                                (int(self.prv_leftMargin + float(a_split[7]) * xSep), int(a_img_height - (self.prv_bottomMargin + (ySep * float(a_split[6]))))),
                                a_pen_color,
                                a_pen_width
                            )
                            a_IsPlot = True
                    ...
                    for a_cnt2 in range(0, a_textSum2):
                        ...
                        if (a_split[2] == a_split2[1].strip()) and (a_split[3] == a_split2[2].strip()) and (a_split[4] == a_split2[3].strip()) and (a_split[5] == a_split2[4].strip()):
                            ...
                            a_textline2[a_cnt2] = \
                                ...
                                "," + str(self.prv_leftMargin + float(a_split[7]) * xSep) + "," + str(a_img_height - (self.prv_bottomMargin + (ySep * float(a_split[6]))))
                    ...
                    if (a_split2[6] != "") and (a_split2[7] != ""):
                        ...
                        if (a_mark == 1):   # 赤○（外側は黒線）
                            ...
                            cv2.putText(
                                h_img,
                                "×",
                                (int(float(a_split2[6]) - a_fnt_font2_size * 10), int(float(a_split2[7]) - a_fnt_font2_size * 10)),
                                cv2.FONT_HERSHEY_PLAIN,
                                a_fnt_font2_size,
                                (0, 0, 255)
                            )
    ...
    def _drawUnreal(
        ...
        try:
            ...
            a_pt = np.array(
                [
                    [int(self.prv_leftMargin), int(a_img_height - self.prv_bottomMargin)],
                    [int(self.prv_leftMargin), int(a_img_height - (self.prv_bottomMargin + (ySep * (self.com.g_yUnit * yLine))))],
                    [int(self.prv_leftMargin + xSep * ((self.com.g_yUnit * yLine) / a_unReal)), int(a_img_height - (self.prv_bottomMargin + (ySep * (self.com.g_yUnit * yLine))))]
                ]
            )
            ...
            # 画像を重ねる
            #a_imgL = cv2.imread(".\\images\\imgUnreal.gif")
            a_imgL = cv2.imread(".\\images\\imgUnreal.png")
            a_height, a_width = a_imgL.shape[:2]    # サイズを取得
            h_img[
            int(self.prv_topMargin + 33.333333333333336):int(self.prv_topMargin + 33.333333333333336) + a_height,
            int(self.prv_leftMargin + 6.666666666666667):int(self.prv_leftMargin + 6.666666666666667) + a_width
            ] = a_imgL
    ...
    def _makeAutoContourOrigin(
        ...
        try:
            a_pixel = np.array([255, 255, 255], np.uint8)
            a_grid = np.array([a_pixel]*(int(self.com.g_ImageFileWidth)), np.uint8)
            a_img = np.array([a_grid]*(int(self.com.g_ImageFileHeight)), np.uint8)
            ...
            '''
            cv2.imshow('Origin', a_img)
            while cv2.waitKey(33) != 27:
                pass
            a_f = "C:\\Users\\hal\\PycharmProjects\\test\\chappy.bmp"
            cv2.imwrite(a_f, a_img)
            '''
    ...
    def _saveContourImage(self, h_symbol, h_img):
        ...
        try:
            # OpenCVでは日本語ファイルがNG
            a_src = ".\\" + self.TargetMeshNo + "-" + str(self.StartYear) + "-" + str(self.EndYear) + ".bmp"
            a_dst = self.com.g_OutPath + "\\" + h_symbol + "-" + self.TargetMeshNo + "-" + str(self.StartYear) + "-" + str(self.EndYear) + ".bmp"
            # 一度カレントに作成
            cv2.imwrite(a_src, h_img)
            # リネームする
